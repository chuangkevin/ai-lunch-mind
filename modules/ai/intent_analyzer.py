...
4. **budget** — 預算資訊。
   ...
   - 解析「100-300」→ {"min": 100, "max": 300, "currency": "TWD"}
   - 解析「想花100元」→ {"max": 100, "currency": "TWD"}
   - 解析「大概兩百」→ {"max": 200, "currency": "TWD"}（注意中文數字轉換）
   - 若未提到預算 → null

5. **estimated_price_range** — 依據預算推估的價格帶。
   - max < 150 → "平價"
   - 150 <= max <= 400 → "中等"
   - max > 400 → "高價"
   - 若無預算資訊，根據食物類型推估（小吃/便當→平價，火鍋/燒烤→中等~高價）

6. **search_radius_hint** — 搜尋半徑建議，根據天氣狀況。
   - 天氣炎熱（溫度>30°C 或流汗指數≥7）或下雨機率>60% → "近距離"
   - 天氣舒適 → "中距離"
   - 天氣涼爽且晴朗 → "遠距離可"
   - 若無天氣資訊 → "中距離"

7. **intent** — 使用者意圖分類，只能是以下四種之一：
   - "search_food_type" — 搜尋特定食物類型（最常見）
   - "location_query" — 純粹詢問某地標附近有什麼吃的（沒有指定食物）
